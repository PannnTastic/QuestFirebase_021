...
class EditViewModel (savedStateHandle: SavedStateHandle, private val repositorySiswa: RepositorySiswa):
    ...
    var uiStateSiswa by mutableStateOf(UIStateSiswa())
        private set
    private val idSiswa: Long = savedStateHandle.get<String>(DestinasiEdit.itemIdArg)?.toLong()?: error("idSiswa tidak d temukan di savedStateHandle")
    ...
    suspend fun editSatuSiswa(){
        if (validasiInput(uiStateSiswa.detailSiswa)){
            try {
                ...
                println("Berhasil Edit Data Siswa $idSiswa")
            }catch (e: Exception){
                println("Gagal Edit Data Siswa $idSiswa")
            }
        }
    }
